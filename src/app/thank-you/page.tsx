...
import { ArrowLeft, CheckCircle2, Clock, Mail, MapPin, Phone } from "lucide-react";
      ...
      <main className="min-h-[70vh] bg-gradient-to-b from-white to-slate-50 flex items-center justify-center px-4 py-16">
        <div className="w-full max-w-xl text-center">
          ...
          <p className="mt-4 text-lg text-slate-600 leading-relaxed">
            Thanks for reaching out. A Kinetic Business specialist will review
            your requirements and follow up within{" "}
            <strong className="text-slate-950">1 business day</strong> with a
            tailored quote.
          </p>
          ...
          <div className="mt-8 grid sm:grid-cols-2 gap-4">
            ...
            <div className="rounded-xl border border-slate-200 bg-white p-5 text-left shadow-sm">
              ...
              <div className="space-y-2">
                ...
                >
                  <Phone className="h-3.5 w-3.5" /> {publicContact.phone.display}
                </a>
                <a
                  href={publicContact.email.href}
                  className="flex items-center gap-1.5 text-sm font-semibold text-blue-600 hover:underline"
                >
                  <Mail className="h-3.5 w-3.5" /> {publicContact.email.display}
                </a>
                <div className="flex items-start gap-1.5 text-sm font-semibold text-slate-600">
                  <MapPin className="mt-0.5 h-3.5 w-3.5 shrink-0" /> {publicContact.address}
                </div>
              </div>
            </div>
          </div>
